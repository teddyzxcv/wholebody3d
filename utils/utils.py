...
def json_loader_part_status(data_path, cv=0):
    input_list = []
    target_list = []
    ...
    with tqdm(total=total_items, desc="Total Progress", position=0) as outer_pbar:
        for part in tqdm(load_part_id, desc="Loading parts", leave=False):
            data = json.load(open(data_path+f'/2Dto3D_train_part{part}.json'))
            length = len(data)
            
            with tqdm(total=length, desc=f"Part {part}", position=1, leave=False) as inner_pbar:
                for i in range(length):
                    sample_2d = torch.zeros(1, 133, 2)
                    sample_3d = torch.zeros(1, 133, 3)
        ...
        outer_pbar.update(total_items - outer_pbar.n)
        outer_pbar.close()
